...
DEVICE0="/dev/allegroDecodeIP0"
DEVICE1="/dev/allegroDecodeIP1"
DEVICE2="/dev/allegroDecodeIP2"
DEVICE3="/dev/allegroDecodeIP3"
...
runGstPipeline () {
	...
	wait $PID
        ...
        if [ $? -ne 0 ]; then
           echo "Error Pipeline failed: Check the codec of Input file."
        fi
}
	...
	case $PROPERTY in
		...
		streamName )                                                                                               
                        if [ -z $STREAM_NAME ]; then                                                
                                echo "No stream type specified for $FILE_NAME hence assuming Single as default stream"         
                                STREAM_TYPE="single"                                                                               
                        fi                                                                                                            
                        ;;
		decInstance )                                                                                               
                        if [ -z $DEC_INSTANCE ]; then                                                
                                echo "No decoder instance specified for $FILE_NAME hence assuming allegrodecIP0 as default stream"         
                                DEC_INSTANCE="allegrodecIP0"                                                                               
                        fi                                                                                                            
                        ;;
		portNum )
			if [ -z $PORT_NUM ]; then
...
DisplayUsageFor () {
	...
	case $PROPERTY in
		...
		codecType )
			...
			;;
		streamName )                                                 
                        echo '  -m or --stream-name              : Type of stream used for the input file'
                        echo '                                   : Possible Values: single/multistream'                    
                        echo '                                   : Default Value: single'
                        ;;
		streamNum )                                                 
                        echo '  -z or --stream-num               : Stream number used for the input file'
                        echo '                                   : Possible Values: 2/3/4 used with multistream'                    
